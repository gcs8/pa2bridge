...
After authenticating to the configured PA2 address, PA2Bridge binds discovery to the TCP socket's actual IPv4 peer and first checks that peer in the local neighbour table. If the PA2 is across a router, the App instead checks authorized connection trackers reported within the previous two hours and requires one unambiguous peer-address-to-MAC match. It verifies through Home Assistant's registry that each accepted tracker belongs to UniFi or UniFi Insights. A MAC is not carried through a Layer 3 connection itself; the network integration supplies that control-plane mapping.
